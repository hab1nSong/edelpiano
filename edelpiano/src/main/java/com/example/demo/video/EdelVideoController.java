package com.example.demo.video;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

import com.example.demo.vo.EdelVideoVO;

@Controller
@RequestMapping("/video")
public class EdelVideoController 
{
	@Autowired
	private EdelVideoDAO dao;
	
	@GetMapping("/")
	public String index() {
		return "Spring boot MyBatis Test";
	}
	
	@GetMapping("/vlogdetail/{video_num}")
	public String VideoTest(@PathVariable("video_num")int video_num, Model model) {
		model.addAttribute("video", dao.selectById(video_num));
		return "Edel_ContentsVlogDetail_Form";
	}
	
	@GetMapping("vloglist")
	public String vlogList(Model model) {
		model.addAttribute("list", dao.getVideoList());
		return "Edel_ContentsVlogList_Form";
	}
	
	/*
	@GetMapping("test")
	public String vlogList(Model model) {
		model.addAttribute("list", dao.getVideoList());
		return "testtest";
	}*/
	
	@GetMapping("/ccWrite")
	public String concoursWriteForm() {
		return "Edel_ContentsVlogWrite_Form";
	}
	
	@PostMapping("/ccWrite")
	@ResponseBody
	public String concoursWrite(EdelVideoVO vo) {
		return String.format("{\"writeOK\":%b}", dao.insert(vo));
	}
	
	@GetMapping("/delete/{video_num}")
	public String delete(@PathVariable("video_num")int video_num) {
		dao.delete(video_num);
		return "redirect:/video/vloglist";
	}
}
